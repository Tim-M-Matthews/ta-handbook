...
import { handbookViewOptionsFromRequest } from "../../lib/handbook-request";
...
export const GET: APIRoute = ({ locals, cookies }) => {
  ...
  const rows = searchRowsForRoles(roles, viewOptions).map((r) => ({
    slug: r.slug,
    title: r.title,
